...
onMounted(() => {
  if (props.marketplace.role.value === "Admin") void props.marketplace.refreshMarketplaceState();
});
onUnmounted(() => {
  window.removeEventListener("hashchange", syncPath);
});

watch([searchTerm, statusFilter, typeFilter, sellerFilter], () => {
  pageNumber.value = 1;
});

const detailsId = computed(() => currentPath.value.match(/^\/transactions\/(.+)$/)?.[1] ?? null);
const detailsItem = computed(() =>
  transactionsView.value.find((item: TransactionRowView) => item.id === detailsId.value) ?? null
);
const totalPages = computed(() => Math.max(1, Math.ceil(transactionsView.value.length / pageSize)));
const pagedItems = computed(() =>
  transactionsView.value.slice((pageNumber.value - 1) * pageSize, (pageNumber.value - 1) * pageSize + pageSize)
);
...
<template>
  <section class="dashboard-screen">
    <PageHeader title="Transactions" subtitle="Monitor request lifecycle, status, and amount flows.">
      <Button variant="secondary" size="sm" @click="marketplace.refreshMarketplaceState">Refresh</Button>
    </PageHeader>

    <Card v-if="detailsId" title="Transaction Details">
      <div class="ui-row-actions">
        <Button @click="goList">Back to Transactions</Button>
      </div>
      <TransactionDetailsPage :item="detailsItem" />
    </Card>

    <Card v-else title="Transaction List">
      <FilterBar>
      ...
      <Pagination
        ...
        :total-items="transactionsView.length"
        :page-size="pageSize"
