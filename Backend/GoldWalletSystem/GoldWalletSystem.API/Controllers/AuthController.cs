...
[ApiController]
[Route("api/auth")]
public class AuthController(IAuthService authService, ICurrentUserService currentUser) : SecuredControllerBase(currentUser)
{
    [HttpGet("ping")]
    ...
    public async Task<IActionResult> Logout([FromBody] LogoutRequestDto? request, CancellationToken cancellationToken = default)
    {
        ...
        await authService.LogoutAsync(CurrentUserId.Value, request?.RefreshToken, cancellationToken);
