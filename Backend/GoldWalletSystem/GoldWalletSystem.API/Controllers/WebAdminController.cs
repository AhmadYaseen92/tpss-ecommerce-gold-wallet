    ...
    IMobileAppConfigurationService mobileAppConfigurationService) : ControllerBase
{
    ...
    public async Task<IActionResult> GetInvoices(CancellationToken cancellationToken)
    {
        ...
        var invoices = invoiceEntities.Select(x => new WebInvoiceDto
        {
            ...
            ProductName = dbContext.WalletAssets.Where(w => w.Id == x.WalletItemId).Select(w => w.ProductName).FirstOrDefault() ?? dbContext.Products.Where(p => p.Id == x.ProductId).Select(p => p.Name).FirstOrDefault() ?? x.ProductName ?? string.Empty,
            ...
            Weight = x.Weight,
            Category = dbContext.WalletAssets.Where(w => w.Id == x.WalletItemId).Select(w => w.Category.ToString()).FirstOrDefault() ?? dbContext.Products.Where(p => p.Id == x.ProductId).Select(p => p.Category.ToString()).FirstOrDefault() ?? x.InvoiceCategory ?? string.Empty,
            Quantity = x.Quantity,
