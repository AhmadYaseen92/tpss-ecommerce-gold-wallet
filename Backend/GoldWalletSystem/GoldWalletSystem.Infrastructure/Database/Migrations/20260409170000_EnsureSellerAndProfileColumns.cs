...
{
    /// <summary>
    /// Repair migration for environments where an empty migration was applied
    /// before seller/profile schema changes were materialized.
    /// </summary>
    public partial class EnsureSellerAndProfileColumns : Migration
    {
...
IF NOT EXISTS (
    ...
    WHERE name = N'FK_Users_Sellers_SellerId')
BEGIN
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.foreign_keys
    WHERE name = N'FK_Products_Sellers_SellerId')
BEGIN
...
");
        ...
        {
            // Intentionally no-op: this is a repair migration for drifted environments.
        }
    }
}
